...
import {
  X, LayoutDashboard, Users, Shield, Key, Globe, ScrollText, Database,
  Server, Activity, Plug, Bell, Palette, Lock, Puzzle, Settings,
  RefreshCw, Search, BarChart3, HelpCircle,
} from "lucide-react";

type Group = "dashboard" | "users" | "security" | "infrastructure" | "integrations" | "settings";

const NAV: { href: string; label: string; labelAr: string; icon: any; group: Group }[] = [
  { href: "/admin/dashboard", label: "System Dashboard", labelAr: "لوحة النظام", icon: LayoutDashboard, group: "dashboard" },

  { href: "/admin/users", label: "User Accounts", labelAr: "حسابات المستخدمين", icon: Users, group: "users" },
  { href: "/admin/rbac", label: "Roles & Permissions", labelAr: "الأدوار والصلاحيات", icon: Shield, group: "users" },
  { href: "/admin/sso", label: "SSO / Authentication", labelAr: "الدخول الموحد", icon: Globe, group: "users" },
  { href: "/admin/api-keys", label: "API Keys", labelAr: "مفاتيح API", icon: Key, group: "users" },

  { href: "/admin/audit", label: "Audit Logs", labelAr: "سجلات المراجعة", icon: ScrollText, group: "security" },
  { href: "/admin/compliance", label: "Compliance", labelAr: "الامتثال", icon: Lock, group: "security" },
  { href: "/admin/incidents", label: "Security Incidents", labelAr: "الحوادث الأمنية", icon: Shield, group: "security" },

  { href: "/admin/system-health", label: "System Health", labelAr: "صحة النظام", icon: Activity, group: "infrastructure" },
  { href: "/admin/database", label: "Database", labelAr: "قاعدة البيانات", icon: Database, group: "infrastructure" },
  { href: "/admin/backup", label: "Backups & Recovery", labelAr: "النسخ الاحتياطي", icon: RefreshCw, group: "infrastructure" },
  { href: "/admin/monitoring", label: "Monitoring", labelAr: "المراقبة", icon: Server, group: "infrastructure" },

  { href: "/admin/integrations", label: "Integrations Hub", labelAr: "مركز التكاملات", icon: Plug, group: "integrations" },
  { href: "/admin/notifications", label: "Platform Notifications", labelAr: "إشعارات المنصة", icon: Bell, group: "integrations" },

  { href: "/admin/branding", label: "Custom Branding", labelAr: "التخصيص", icon: Palette, group: "settings" },
  { href: "/admin/data-governance", label: "Data Governance", labelAr: "حوكمة البيانات", icon: Lock, group: "settings" },
  { href: "/admin/extensions", label: "Extensions", labelAr: "الإضافة", icon: Puzzle, group: "settings" },
  { href: "/admin/settings", label: "System Settings", labelAr: "إعدادات النظام", icon: Settings, group: "settings" },
];
...
const GROUP_META: Record<Group, { en: string; ar: string }> = {
  ...
  security: { en: "Security & Audit", ar: "الأمن والمراجعة" },
  infrastructure: { en: "Infrastructure", ar: "البنية التحتية" },
  integrations: { en: "Integrations", ar: "التكاملات" },
  settings: { en: "Settings & Governance", ar: "الإعدادات والحوكمة" },
};
      ...
      <aside className={cn("fixed lg:sticky top-0 z-50 lg:z-30 h-screen w-72 shrink-0 border-r border-border bg-background/60 backdrop-blur-2xl shadow-[1px_0_40px_rgba(0,0,0,0.03)] dark:shadow-[1px_0_40px_rgba(255,255,255,0.02)] flex flex-col transition-transform duration-300", sidebarOpen ? "translate-x-0" : "-translate-x-full lg:translate-x-0", ar && "lg:order-last border-r-0 border-l")}>
        <div className="flex items-center justify-between px-5 h-16 border-b border-border">
          <Link href="/admin/dashboard" className="flex items-center gap-2">
            ...
            <div className="flex flex-col leading-none">
              ...
              <span className="text-[10px] text-muted-foreground font-arabic">{ar ? "مدير النظام" : "System Admin"}</span>
            </div>
          </Link>
        ...
        <nav className="flex-1 overflow-y-auto scrollbar-iscarb px-3 py-4">
          <ul className="space-y-6">
            {(["dashboard", "users", "security", "infrastructure", "integrations", "settings"] as Group[]).map((groupKey) => (
              <li key={groupKey}>
        ...
        <div className="border-t border-border p-4">
          <div className="rounded-xl bg-gradient-to-br from-[#0E6C3C]/10 to-[#0F7B8A]/10 border border-[#0E6C3C]/20 p-4 text-center shadow-inner">
            <p className="font-display text-xs font-bold text-[#0E6C3C] dark:text-[#58CE95] leading-tight tracking-wide uppercase">{ar ? "إدارة تقنية" : "Platform Operations"}</p>
          </div>
        </div>
      </aside>
    </>
