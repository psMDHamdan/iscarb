...
import {
  X, LayoutDashboard, FileCode, Package, Puzzle, Link as LinkIcon,
  Terminal, GitBranch, TestTube, BarChart3, Users, Settings,
} from "lucide-react";

type Group = "dashboard" | "apis" | "tools" | "community";

const NAV: { href: string; label: string; labelAr: string; icon: any; group: Group }[] = [
  { href: "/developer/dashboard", label: "Developer Dashboard", labelAr: "لوحة المطور", icon: LayoutDashboard, group: "dashboard" },

  { href: "/developer/api-docs", label: "API Documentation", labelAr: "وثائق API", icon: FileCode, group: "apis" },
  { href: "/developer/sdks", label: "SDKs", labelAr: "مكتبات SDK", icon: Package, group: "apis" },
  { href: "/developer/webhooks", label: "Webhooks", labelAr: "الويب هوكس", icon: LinkIcon, group: "apis" },
  { href: "/developer/api-keys", label: "API Keys", labelAr: "مفاتيح API", icon: Settings, group: "apis" },

  { href: "/developer/plugins", label: "Plugin Marketplace", labelAr: "سوق الإضافات", icon: Puzzle, group: "tools" },
  { href: "/developer/cli", label: "CLI / SDK", labelAr: "سطر الأوامر", icon: Terminal, group: "tools" },
  { href: "/developer/sandbox", label: "Test Sandbox", labelAr: "بيئة الاختبار", icon: TestTube, group: "tools" },
  { href: "/developer/version-control", label: "Version Control", labelAr: "التحكم بالإصدارات", icon: GitBranch, group: "tools" },

  { href: "/developer/api-usage", label: "API Usage", labelAr: "استخدام API", icon: BarChart3, group: "community" },
  { href: "/developer/contributions", label: "Contributions", labelAr: "المساهمات", icon: Users, group: "community" },
];

const GROUP_META: Record<Group, { en: string; ar: string }> = {
  dashboard: { en: "Dashboard", ar: "لوحة القيادة" },
  apis: { en: "APIs & Webhooks", ar: "واجهات API والويب هوكس" },
  tools: { en: "Developer Tools", ar: "أدوات المطور" },
  community: { en: "Community & Usage", ar: "المجتمع والاستخدام" },
};
      ...
      <aside className={cn("fixed lg:sticky top-0 z-50 lg:z-30 h-screen w-72 shrink-0 border-r border-border bg-background/60 backdrop-blur-2xl shadow-[1px_0_40px_rgba(0,0,0,0.03)] dark:shadow-[1px_0_40px_rgba(255,255,255,0.02)] flex flex-col transition-transform duration-300", sidebarOpen ? "translate-x-0" : "-translate-x-full lg:translate-x-0", ar && "lg:order-last border-r-0 border-l")}>
        <div className="flex items-center justify-between px-5 h-16 border-b border-border">
          <Link href="/developer/dashboard" className="flex items-center gap-2">
            ...
            <div className="flex flex-col leading-none">
              ...
              <span className="text-[10px] text-muted-foreground font-arabic">{ar ? "بوابة المطور" : "Developer Portal"}</span>
            </div>
          </Link>
        ...
        <nav className="flex-1 overflow-y-auto scrollbar-iscarb px-3 py-4">
          <ul className="space-y-6">
            {(["dashboard", "apis", "tools", "community"] as Group[]).map((groupKey) => (
              <li key={groupKey}>
        ...
        <div className="border-t border-border p-4">
          <div className="rounded-xl bg-gradient-to-br from-[#0E6C3C]/10 to-[#0F7B8A]/10 border border-[#0E6C3C]/20 p-4 text-center shadow-inner">
            <p className="font-display text-xs font-bold text-[#0E6C3C] dark:text-[#58CE95] leading-tight tracking-wide uppercase">{ar ? "ابنِ وتكامل" : "Build & Integrate"}</p>
          </div>
        </div>
      </aside>
    </>
